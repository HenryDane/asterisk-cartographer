...
int do_quests(){
    ...
    quest_file << endl << "#include \"main.h\"" << endl;
    ...
    quest_txt >> str;
    if (str != "STR"){
        cout << "Bad header to string data; expected [STR], found [" << str << "]" << endl;
        return -192;
    }
    int num_str;
    int str_dat_len = 0;
    ...
    ofstream quest_string;
    quest_string.open("quest_string.dat");
    if (!quest_string.is_open()){
        cout << "Failed to open/create quest_string.dat" << endl;
        return -131;
    }
    quest_string << "\"";
    for(int i = 0; i < num_str + 1; i++){ /* yes the plus one needs to be there because reading is hard */
        //quest_txt >> str;
        quest_txt.getline(tmpt, 79);
        cout << "=== Read line ===" << endl << tmpt << endl << "===" << strlen(tmpt) << "===" << endl;
        str_dat_len += strlen(tmpt) + 1;
        quest_string << tmpt;
        quest_string << "\\0";
    }
    quest_string << "\"";
    ...
    quest_file << "/* #define QUEST_STR_DAT_LEN " << str_dat_len << "*/" << endl;

    quest_file << endl;

    quest_file << "char quest_str_dat [ QUEST_STR_DAT_LEN ] = " << endl;
    quest_file << "    #include \"quest_string.dat\"" << endl;
    quest_file << ";" << endl;
    ...
    quest_file << "extern int quests_consumed[NUM_QUESTS_MAX];" << endl;
    quest_file << endl;
